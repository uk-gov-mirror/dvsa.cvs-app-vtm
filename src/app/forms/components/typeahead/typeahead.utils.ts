...
export function createParamsForQuery(query: string, queryParamKey = 'q', customParams = {}) {
	const searchParams = {
		[queryParamKey]: query,
		...customParams,
	};
	...
	const setParam = (acc: HttpParams, param: string) => {
		return acc.set(param, searchParams[param as keyof typeof searchParams] || '');
	};
...
export function resolveApiMethod(method = '') {
	const isMethodValid = ['get', 'post', 'put', 'delete', 'patch', 'request'].some(
